...
class SpotifyClient:
    ...
    def __init__(self, username, password, data_path):
        ...
        self.session = self._get_spotify_session(data_path)
        self._logger = logging.getLogger(__name__)
        self.download_root = os.path.join(data_path, "download")
    ...
    def _get_spotify_session(self, data_path):
        """Initialize the Spotify session.

        Args:
            data_path: The path to the data directory.
        """
        cred_location = os.path.join(data_path, "credentials.json")
    ...
    def get_track_metadata(self, track_id) -> SpotifyTrackMetadata:
        ...
        try:
            ...
            raise ValueError(f"Failed to parse TRACKS_URL [ '{track_id}' ] response: {str(e)}")
    ...
    def process_track(self, track_metadata: SpotifyTrackMetadata) -> SpotifyTrackMetadata:
        ...
        Args:
            track_id: The id of the track to download.

        Returns:
            bytes: The downloaded track.
        """
        try:
